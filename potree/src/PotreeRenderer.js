...
export class Renderer {
	...
	renderNodes(octree, nodes, visibilityTextureData, camera, target, shader, params) {
		...
		for (let node of nodes) {
			...
			{ // Delete Polygons
				if(material.deletePolygons && material.deletePolygons.length > 0){

					let deletePolygonVCount = [];
					let deleteWorldViewProjMatrices = [];

					for(let deletePolygon of material.deletePolygons){
						let view = deletePolygon.viewMatrix;
						let proj = deletePolygon.projMatrix;
						let worldViewProj = proj.clone().multiply(view).multiply(world);

						deletePolygonVCount.push(deletePolygon.markers.length);
						deleteWorldViewProjMatrices.push(worldViewProj);
					}

					let flattenedDeleteMatrices = [].concat(...deleteWorldViewProjMatrices.map(m => m.elements));
					let flattenedDeleteVertices = new Array(8 * 3 * material.deletePolygons.length);
					for(let i = 0; i < material.deletePolygons.length; i++){
						let deletePolygon = material.deletePolygons[i];
						for(let j = 0; j < deletePolygon.markers.length; j++){
							flattenedDeleteVertices[i * 24 + (j * 3 + 0)] = deletePolygon.markers[j].position.x;
							flattenedDeleteVertices[i * 24 + (j * 3 + 1)] = deletePolygon.markers[j].position.y;
							flattenedDeleteVertices[i * 24 + (j * 3 + 2)] = deletePolygon.markers[j].position.z;
						}
					}

					const lDeletePolygonVCount = shader.uniformLocations["uDeletePolygonVCount[0]"];
					gl.uniform1iv(lDeletePolygonVCount, deletePolygonVCount);

					const lDeletePolygonVP = shader.uniformLocations["uDeletePolygonWVP[0]"];
					gl.uniformMatrix4fv(lDeletePolygonVP, false, flattenedDeleteMatrices);

					const lDeletePolygons = shader.uniformLocations["uDeletePolygonVertices[0]"];
					gl.uniform3fv(lDeletePolygons, flattenedDeleteVertices);
				}
			}
	...
	renderOctree(octree, nodes, camera, target, params = {}){
		...
		{ // UPDATE SHADER AND TEXTURES
			...
			//if(material.needsUpdate){
			{
				...
				let numDeletePolygons = (material.deletePolygons && material.deletePolygons.length) ? material.deletePolygons.length : 0;
				...
				let defines = [
					...
					`#define num_deletepolygons ${numDeletePolygons}`,
				];
