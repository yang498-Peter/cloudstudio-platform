...
#if defined(num_deletepolygons) && num_deletepolygons > 0
	uniform int uDeletePolygonVCount[num_deletepolygons];
	uniform vec3 uDeletePolygonVertices[num_deletepolygons * 8];
	uniform mat4 uDeletePolygonWVP[num_deletepolygons];
#endif
...
#if defined(num_deletepolygons) && num_deletepolygons > 0
bool pointInDeletePolygon(vec3 point, int polyIdx) {

	mat4 wvp = uDeletePolygonWVP[polyIdx];
	vec4 pointNDC = wvp * vec4(point, 1.0);
	pointNDC.xy = pointNDC.xy / pointNDC.w;

	int j = uDeletePolygonVCount[polyIdx] - 1;
	bool c = false;
	for(int i = 0; i < 8; i++) {
		if(i == uDeletePolygonVCount[polyIdx]) {
			break;
		}

		vec3 verti = uDeletePolygonVertices[polyIdx * 8 + i];
		vec3 vertj = uDeletePolygonVertices[polyIdx * 8 + j];
...
void doClipping(){
	...
	#if defined(num_deletepolygons) && num_deletepolygons > 0
		for(int i = 0; i < num_deletepolygons; i++) {
			if(pointInDeletePolygon(position, i)){
