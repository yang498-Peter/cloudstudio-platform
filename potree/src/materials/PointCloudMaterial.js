...
export class PointCloudMaterial extends THREE.RawShaderMaterial {
	constructor (parameters = {}) {
		...
		this.uniforms = {
			...
			deletePolygons:		{ type: "3fv", value: [] },
			deletePolygonVCount:{ type: "iv", value: [] },
			deletePolygonVP:	{ type: "Matrix4fv", value: [] },
	...
	setDeletePolygons(deletePolygons) {
		if(!deletePolygons){
			return;
		}

		let previousLength = this.deletePolygons.length;
		this.deletePolygons = deletePolygons;

		if(previousLength !== this.deletePolygons.length){
			this.updateShaderSource();
		}
	}
