...
MemoryData getMemoryData() {
	...
	data.physical_total = (size_t)memsize;

	// Process memory (resident + virtual)
	task_vm_info_data_t vmInfo;
	mach_msg_type_number_t count = TASK_VM_INFO_COUNT;
	if (task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&vmInfo, &count) == KERN_SUCCESS) {
		data.virtual_usedByProcess = (size_t)vmInfo.virtual_size;
		data.physical_usedByProcess = (size_t)vmInfo.phys_footprint;
		data.virtual_usedByProcess_max = data.virtual_usedByProcess;
		data.physical_usedByProcess_max = data.physical_usedByProcess;
	}

	// System-wide values not critical for converter logic; keep best-effort.
	data.virtual_total = data.physical_total;
	data.virtual_used = data.virtual_usedByProcess;
	data.physical_used = data.physical_usedByProcess;
...
CpuData getCpuData() {
	CpuData data;
	data.numProcessors = std::thread::hardware_concurrency();
	data.usage = 0.0;
	return data;
}
